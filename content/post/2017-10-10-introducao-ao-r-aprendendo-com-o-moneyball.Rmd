---
title: Introdução ao R - Aprendendo com o Moneyball
author: Pedro Nascimento de Lima
date: '2017-10-10'
slug: introducao-ao-r-aprendendo-com-o-moneyball
categories:
  - R
tags: []
header:
...
Normalmente lemos dados no formato .csv no R para realizar as análises. É possível também ler dados em outros formatos. Obtenha [aqui o arquivo baseball](/post/2018-10-10-moneyball-r/baseball.csv).

```{r}
# Lendo Dados em CSV
baseball <- read.csv("2018-10-10-moneyball-r/baseball.csv")
```
...
### Realizando um teste de Correlação
...
Também podemos visualizar que existe uma correlação alta entre estas duas variáveis utilizando gráficos da biblioteca ggpubr.
...
$$ Vitorias = \beta_0 + \beta_1 * RunDiffs + e$$
...
$$ Vitorias = 80.88 + 0.1057 RunDiffs$$
```{r}
summary(modelo_vitorias)
```

### Será que é são necessários 135 Runs a mais para chegar à Playoffs?
A partir da regressão linear nós estimamos que $ Vitorias = 80.88 + 0.1057 RunDiffs$ e também sabemos que $Vitorias >= 95$.
...
$$ Runs = -804.3 + 2737.77 * OBP + 1584.91 * SLG$$
```{r, size = 'Tiny'}
...
$$ Runs Permitidas = -837 + 2913.6 * OOBP + 1514.29 * OSLG$$
```{r, size = 'Tiny'}
...
- Nossa Regressão para Runs foi $ Runs = -804.3 + 2737.77 * OBP + 1584.91 * SLG$.
...
- Nossa Regressão para Runs foi $ Runs Permitidas = -837 + 2913.6 * OOBP + 1514.29 * OSLG$.
...
- Nosso modelo de vitórias diz que $ Vitorias = 80.88 + 0.1057 RunDiffs$. Então..
...
```{r, eval= F, table2, echo=FALSE, message=FALSE, warnings=FALSE, results='asis'}
tabl <- "
|Variável|Nosso Modelo | Modelo do Paul | Realizado |
|--------------|-------------------|-----------------------|---------------|
|Runs | 805 | 800 - 820 | 800 |
