---
title: Discrete Event Simulation Metamodeling - Splines with R and Arena
author: Pedro N. de Lima
date: '2019-04-01'
slug: discrete-event-simulation-metamodeling-splines-r-arena
categories:
  - R Blogs
  - R
tags:
  ...
  - R
header:
  caption: 'Learn how to use R to build Spline Metamodels of Arena Simulation Models.'
  image: ''
...
You can build a spline model with the same linear model `lm` function. Instead of using the standard `Y ~ X` formula, we just have to use the `bs()` function from the `splines` package. Thus, our formula for our spline metamodel will be `Y ~ bs(X)`.
...
The suggested batch size is `r Optimum_BatchSize`. Is this a reasonable guess, based our simulation runs? Let's figure this out by plotting the simulation data, the spline function and the optimum value found.
